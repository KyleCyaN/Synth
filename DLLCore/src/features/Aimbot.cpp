...
#include <algorithm>
#include "../utils/SynthUtils.h"          // ← 统一入口
#include "features.h"
#include "memory.h"
#include <cmath>
#include "Bones.h"
...
// ⚠️ 删除: static LPVOID AllocateNear(...)  → SynthUtils
...
// ⚠️ 删除: static bool IsAimingDownSight()  → SynthUtils
// ⚠️ 删除: static bool WorldToScreen(...)   → SynthUtils
// ⚠️ 删除: static bool GetPlayerHeadBone()  → SynthUtils
// ⚠️ 删除: Aimbot::PI, Aimbot::Normalize()  → SynthUtils
...
bool Aimbot::InstallPitchHook() {
    ...
    if (!g_pitchShellcode) return false;
    ...
    DWORD oldProt;
    VirtualProtect(reinterpret_cast<LPVOID>(g_pitchHookAddr), 8,
                   PAGE_EXECUTE_READWRITE, &oldProt);
    BYTE jmp[5] = {0xE9};
    const auto rel = static_cast<int32_t>(
        reinterpret_cast<uintptr_t>(g_pitchShellcode) - (g_pitchHookAddr + 5));
    memcpy(&jmp[1], &rel, 4);
...
void Aimbot::Run() {
    ...
    for (const auto& p : players) {
        ...
        if (float screenDist = sqrtf(dx * dx + dy * dy); screenDist < bestScreenDist) {
    ...
    float newYaw = NormalizeAngle(currentYaw + neededDeltaYaw * smooth);
